...
def test_lattice():
    ...
    angles = Vector3D(values=[Degree(90), Degree(90), Degree(90)])
    parameters = Vector3D(values=[Angstrom(2), Angstrom(2), Angstrom(2)])
    atoms = [
        Atom(position=Vector3D(values=[Angstrom(
            0), Angstrom(0), Angstrom(0)])),
        Atom(position=Vector3D(values=[Angstrom(
